...
export default function ListeningPartyHub() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-700 to-indigo-900 pt-20 pb-32 shadow-2xl">
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 h-[500px] w-[500px] bg-white/10 blur-[120px] rounded-full" />
        <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/4 h-[400px] w-[400px] bg-purple-400/20 blur-[100px] rounded-full" />
        
        <div className="container relative px-4 mx-auto">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 text-white text-xs font-bold mb-8 backdrop-blur-md animate-in fade-in slide-in-from-bottom-2">
              <Radio className="h-3.5 w-3.5 animate-pulse" />
              LIVE INTERACTION
            </div>
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-white mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              Listening <span className="text-primary-200">Party</span>
            </h1>
            <p className="text-xl md:text-2xl text-primary-50/80 mb-12 max-w-2xl font-medium leading-relaxed animate-in fade-in slide-in-from-bottom-6 duration-700">
              Experience music together in real-time. Join public sessions or create your own private room for friends.
            </p>
            
            <div className="flex flex-wrap gap-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
              <button
                onClick={() => setIsCreateModalOpen(true)}
                className="w-full md:w-auto group flex items-center justify-center gap-3 px-10 py-5 rounded-2xl bg-white text-primary-600 font-black hover:bg-primary-50 transition-all shadow-2xl shadow-black/20 active:scale-95"
              >
                <Plus className="h-6 w-6" />
                Create Your Party
              </button>
              
              <form onSubmit={handleJoinByCode} className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                <div className="relative group w-full md:w-auto">
                  <input
                    ...
                    maxLength={6}
                    className="w-full md:w-64 px-6 py-5 rounded-2xl bg-white/10 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all font-mono font-black tracking-[0.2em] text-center text-white placeholder:text-white/30 text-lg backdrop-blur-md"
                  />
                </div>
                <button
                  type="submit"
                  disabled={isJoining || roomCode.length !== 6}
                  className="w-full md:w-auto p-5 rounded-2xl bg-white/10 border border-white/20 hover:bg-white/20 text-white transition-all disabled:opacity-30 backdrop-blur-md shadow-xl flex items-center justify-center"
                >
                  {isJoining ? <Loader2 className="h-6 w-6 animate-spin" /> : <ArrowRight className="h-6 w-6" />}
                </button>
              </form>
            </div>
          </div>
      ...
      <div className="container px-4 mx-auto -mt-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="p-8 rounded-[2.5rem] bg-card border border-border shadow-xl space-y-8 sticky top-24">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            {isLoadingJams ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-6 rounded-3xl bg-white/[0.02] border border-dashed border-white/10 text-center">
                <div className="h-20 w-20 rounded-full bg-white/5 flex items-center justify-center mb-6">
                  <Radio className="h-10 w-10 text-muted-foreground opacity-20" />
                </div>
                <h3 className="text-xl font-bold mb-2">No public parties yet</h3>
